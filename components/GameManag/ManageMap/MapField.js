...
const MapField = ({ fieldsRef, map, x, y, dragRef }) => {
  const [field, setField] = React.useState(map.coords.filter(f=>f.coords.x === x && f.coords.y === y))

  const handleClick = async ()=> {
    if(dragRef.current) return // if we dragging, do nothing
  ...
  return (
    <>
      <div className="map-col" key={Math.random()} onClick={()=>handleClick()} >
        {field?.map((f,i)=>{
          return <Image 
            style={{pointerEvents:"none", zIndex: f.layer}}
            key={i}
            className={`map-field rotate-${f.rotation}${f.flip ? "-flip" : ""}`} src={f.imageSrc}
          ></Image>
        })
        }
      </div>
